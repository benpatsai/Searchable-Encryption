...
const char *http_request_headers(int fd)
{
    ...
    for (;;)
    {
        ...
        *colon = '\0';
        ...
        url_decode(value, sp, 512); //bug 4 line.159
        ...
        if (strcmp(buf, "CONTENT_TYPE") != 0 &&
            ...
            snprintf(envvar, 512, "HTTP_%s", buf); // bug 5 line.165
...
void http_serve(int fd, const char *name)
{
    ...
    strncat(pn, name, sizeof(pn)-strlen(pn)); //bug1 line.255
    split_path(pn);
...
void dir_join(char *dst, const char *dirname, const char *filename) {
    strncpy(dst, dirname, strlen(dirname)>=sizeof(dst)? strlen(dirname) : sizeof(dst)); //bug2 line.317
    if (dst[strlen(dst) - 1] != '/')
        strcat(dst, "/");
    strcat(dst, filename); //bug3 line.320
}

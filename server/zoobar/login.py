...
class User(object):
    ...
    def addRegistration(self, username, password):
        ...
        isNewBank = bank_client.new_user(username)
...
def login():
    ...
    if request.method == 'POST':
        ...
        if 'submit_registration' in request.form:
            if not username:
                login_error = "ERROR: You must supply a username to register."
            elif not password:
                login_error = "ERROR: You must supply a password to register."
            else:
            ...
            if not username:
                login_error = "ERROR: You must supply a username to log in."
            elif not password:
                login_error = "ERROR: You must supply a password to log in."
            else:
                cookie = user.checkLogin(username, password)
                hash_client.load_user(username)
                if not cookie:
